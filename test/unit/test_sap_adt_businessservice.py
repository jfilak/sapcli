...
class TestbusinessserviceBinding(unittest.TestCase):
    ...
    def test_service_published_by_name(self):
        ...
        binding = sap.adt.businessservice.ServiceBinding(connection, binding_name)
        binding.fetch()
        status = binding.publish(service_name)
    ...
    def test_service_published_by_version(self):
        connection = sample_connection_ok()
        ...
        binding = sap.adt.businessservice.ServiceBinding(connection, binding_name)
        binding.fetch()
        status = binding.publish(None, service_version)
    ...
    def test_service_published_single(self):
        connection = sample_connection_ok(RESPONSE_BINDING_OK_SINGLE_SRVD)

        binding_name = 'TEST_BINDING'

        binding = sap.adt.businessservice.ServiceBinding(connection, binding_name)
        binding.fetch()
        status = binding.publish()
    ...
    def test_service_not_found(self):
        ...
        with self.assertRaises(sap.errors.SAPCliError):
            binding.publish(None, service_version)
