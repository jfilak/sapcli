...
from sap.rest.gcts.sugar import LogSugarOperationProgress
...
def clone(connection, args):
    ...
    console = sap.cli.core.get_console()
    need_to_check_activities = not (args.no_import or args.buffer_only)
    clone_action = None
    sync_clone_exc = None
    ...
    if args.sync_clone:
        ...
        parameters['progress_consumer'] = LogSugarOperationProgress()
    else:
    ...
    try:
        ...
        if args.wait_for_ready > 0 and args.sync_clone:
            ...
            # save exception for later use after checking clone process
            sync_clone_exc = exc
        else:
    ...
    # make sure that we have repository is cloned  for both cases: sync and async clone
    if need_to_check_activities and not is_cloned_activity_success(console, repo):
        if sync_clone_exc:
            console.printerr(str(sync_clone_exc))
        return 1
    # check if repository is cloned for async clone without checking activities
    # dont check it for  sync clone because it will be checked in is_clone_done in next if statement
    if not args.sync_clone and not need_to_check_activities:
        repo.refresh()
        # figure out is refreshed repository is cloned
        if not repo.is_cloned:
            console.printout('Repository has not been cloned')
            return 1

    # wait for repository to be ready for sync clone with error and check
    if args.sync_clone and args.wait_for_ready > 0 and sync_clone_exc:
        ...
        with sap.cli.helpers.ConsoleHeartBeat(console, args.heartbeat):
            ...
            sap.rest.gcts.simple.wait_for_operation(repo, is_clone_done, args.wait_for_ready, sync_clone_exc)
    ...
    try:
        ...
        if args.wait_for_ready > 0:
            ...
            if need_to_check_activities:
                ...
                # TODO: or enough to check it once?
                # or need to check it in Loop until the repository is checked out or wait_for_ready timeout is reached?
                repo.refresh()
                if repo.branch != args.branch:
                    console.printerr(str(exc))
                    return 1
