...
def inactiveobjects_list(connection, args):
    ...
    def print_entry(entry, prefix=''):
        printout(f'{prefix}{entry.object.reference.name} ({entry.object.reference.typ})')
    ...
    for root_entry in inactive_results.entries:
        if root_entry.object.reference.parent_uri:
            continue

        root_entry_uri = root_entry.object.reference.uri
        ...
        for child_entry in inactive_results.entries:
            if root_entry_uri != child_entry.object.reference.parent_uri:
                continue

            child_uri = child_entry.object.reference.uri
    ...
    for leftover_entry in inactive_results.entries:
        leftover_uri = leftover_entry.object.reference.uri
