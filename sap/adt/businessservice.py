...
class ServiceBinding(ADTObject):
    ...
    def publish(self, service_name=None, service_version=None):
        """Publish service definition"""

        service = None
        if service_name is not None:
            service = next(
                (item for item in self.services if item.definition.name == service_name), None)
        elif service_version is not None:
            service = next(
                (item for item in self.services if item.version == service_version), None)
        elif len(self.services) == 1:
            service = self.services[0]

        if service is None:
            # pylint: disable=line-too-long
            raise SAPCliError(f'Business Service Binding {self.name} has no Service Definition with supplied name or version')
